function [A, At, Gw] = generate_data_basic(Nx,Ny,f,super_res,seed)
% Generates A, At and Gw matrix given:
% Nx, Ny: picture size
% f: observation frequency
% Addiditonal parameters are:
% super_res: indicates if we are in the case of super_resolution
% (super_res=1) or not (super_res=0).
% seed: random seed for random number generation. This is important only in
% the case of multispectral imaging where the uv coverage has to be similar
...
param_HSI.uv = 'realistic';
param_HSI.percentage = 100;

generate_simple_image = 1;
...
%%
if strcmp(param_HSI.uv,'gaussian')
...
%%
if strcmp(param_HSI.uv,'realistic')
    param_data.cov_type = 'random'; 'vlaa';
    param_data.T = param_HSI.percentage;
    param_data.na = 25;
    
    if super_res==1 % in this case we want the same coverages for each observation
        rng(seed);
    end
